...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <telerik:RadCodeBlock ID="RadCodeBlock1" runat="server">
        <script type="text/javascript">
        	...
        	function ChangeImgStatus(IsLoad) {
        		...
        		if (ischkUseImg) {
        		    $("#imgRot").attr("disabled", false);
        		    $("#<%=ddlImgRotation.ClientID %>").attr("disabled", false);
        		    $("#<%=ddlimageTransparency.ClientID %>").attr("disabled", false);
        			$("#imgPosition").attr("disabled", false);
        			rdoImgPosition.prop("disabled", false);
                }
                else {
                    $("#imgRot").attr("disabled", true);
                    $("#<%=ddlImgRotation.ClientID %>").attr("disabled", true);
        		    $("#<%=ddlimageTransparency.ClientID %>").attr("disabled", true);
        			$("#imgPosition").attr("disabled", true);
            ...
            function ChangeDlUser() {
        	    ...
        	    if (ischkDlUser) {
        	        $("#txtSetting").attr("disabled", false);
        	        $("#txtPosition").prop("disabled", false);
        	        $("#<%=ddlTxtRotation.ClientID %>").attr("disabled", false);
        			if (ColorPicker1 != null)
        			    ColorPicker1.set_enabled(true);
        			$("#<%=ddlTxtSize.ClientID %>").attr("disabled", false);
        		    if (Transparency_sys)
        		        $("#<%=ddlTransparency1.ClientID %>").attr("disabled", false);
                    rdoTxtPosition.prop("disabled", false);
                }
                else {
                    $("#txtSetting").attr("disabled", true);
                    $("#txtPosition").prop("disabled", true);
                    $("#<%=ddlTxtRotation.ClientID %>").attr("disabled", true);
        			if (ColorPicker1 != null)
        			    ColorPicker1.set_enabled(false);
        			$("#<%=ddlTxtSize.ClientID %>").attr("disabled", true);
        		    if (Transparency_sys)
        		        $("#<%=ddlTransparency1.ClientID %>").attr("disabled", true);
                    rdoTxtPosition.prop("disabled", true);
                }
            }
            ...
            function ChangeDefText() {
        	    ...
        	    if (ischkDefText) {
        	        $("#txtSetting2").attr("disabled", false);
        	        $("#txtPosition2").attr("disabled", false);
        	        $("#<%=ddlTxtRotation2.ClientID %>").attr("disabled", false);
        			if (ColorPicker2 != null)
        			    ColorPicker2.set_enabled(true);
        			$("#<%=ddlTxtSize2.ClientID %>").attr("disabled", false);
        		    $("#<%=txtDefText.ClientID %>").attr("disabled", false);
        		    if (Transparency_def)
        		        $("#<%=ddlTransparency2.ClientID %>").attr("disabled", false);
                    rdoTxtPosition2.prop("disabled", false);
        		}
        		else {
        		    $("#txtSetting2").attr("disabled", true);
        		    $("#txtPosition2").attr("disabled", true);
        		    $("#<%=ddlTxtRotation2.ClientID %>").attr("disabled", true);
        			if (ColorPicker2 != null)
        			    ColorPicker2.set_enabled(false);

        			$("#<%=ddlTxtSize2.ClientID %>").attr("disabled", true);
        		        $("#<%=txtDefText.ClientID %>").attr("disabled", true);
        		    if (Transparency_def)
        		        $("#<%=ddlTransparency2.ClientID %>").attr("disabled", true);
                    rdoTxtPosition2.prop("disabled", true);
        		}
            }
            ...
            function btnPDFPreview_Click() {
                ...
                if (ischkUseImg2) {
                    if ( fs == null && fs.get_count() <= 0) {
                        alert($("#<%=hidNeedUpLoad.ClientID %>").val());
        			    return;
        			}

                    if (hfFileIsExist == 'False') {
                        alert($("#<%=hfNoFindFile.ClientID %>").val());
        			    return;
                    }
                    var files = fs.get_list();
                    fileid = files[0].Id;
        ...
        <table style="width: 70%; text-align: left; margin: 0 auto" class="tab">
            ...
            <tr>
                <td style="background-color: #ffffff; width: 50%" valign="top">
                    <div id="divTxtDL">
                        <!--<asp:RadioButton ID="rdoDlUser" runat="server" Checked="True" GroupName="WaterGroup"
                    Text="下載者的姓名及部門" meta:resourcekey="rdoDlUserResource1" />-->
                        <br />
                        <asp:CheckBox ID="chkDlUser" runat="server" Text="下載者的姓名及部門" meta:resourcekey="chkDlUserResource1" />&nbsp;
                    </div>
                </td>
            ...
            <tr>
                <td style="background-color: #ffffff; width: 50%">
                    <div id="imgWater">
                        <uc5:UC_FileCenter runat="server" ID="UC_FileCenter" ProxyEnabled="true" />
                        &nbsp;
                    </div>
    ...
    <asp:Label ID="labChangeSetup" runat="server" Text="是否要改變浮水印的設定?" Visible="False" meta:resourcekey="labChangeSetupResource1"></asp:Label></center>
        <input id="hidNeedUpLoad" runat="server" type="hidden" value="請先上傳檔案" meta:resourcekey="hidNeedUpLoadResource1" />
    <input id="hfNoFindFile" runat="server" type="hidden" value="找不到該圖形浮水印檔案" meta:resourcekey="hfNoFindFileResource1" />
    <asp:Label ID="lblMsgText" runat="server" Text="請填寫自訂浮水印文字內容" Visible="False"
        meta:resourcekey="lblMsgTextResource1"></asp:Label>
