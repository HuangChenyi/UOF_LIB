...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <script type="text/javascript">
        function ConfirmApprove() {
            ...
            //檢查是否有重複的文件名稱
            var nameData = [$('#<%=lblDocname.ClientID %>').text(), escape('<%=docid %>'), $('#<%=hdFolderID.ClientID %>').val()];
            ...
            if (ApproveValue[2] == "WKF" && ApproveValue[1] == "true" && IsSkipApprove != "true") {
                ...
                var taskdata = [FormVersionId, DocId, VersionMemos, ManualVersion, SysVersion, ActualVersion, '<%=lblDocLink.Text%>', '<%=lblHistory.Text%>', '<%=lblRef.Text%>', '<%=lblInfo.Text%>', '<%=labNormal.Text%>', '<%=lblSecretm.Text%>', '<%=lblTopSecret.Text%>', '<%=lblXSecret.Text%>', '<%=lblXXSecret.Text%>', '<%=lblUrgencyNormal.Text%>', '<%=lblUrgencyHigh.Text%>', '<%=lblUrgencyExHigh.Text%>', DocSerial];
    ...
    <input id="hdApproveType" runat="server" type="hidden" />
    <input id="hdMemo" runat="server" type="hidden" />
    <input id="hdKeyWord" runat="server" type="hidden" />
