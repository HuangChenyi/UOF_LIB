...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
        ...
        function RadToolBar1_ButtonClicking(sender, args) {
            ...
            if (keyValue == "New") {
                ...
                $uof.dialog.open2("~/DMS/DocStore/DocPrintRetrieve.aspx", args.get_item(), "", 500, 400, function (returnValue) {
                    ...
                    return true;  
                }, { "detailGuid": '<%=Request["detailId"]%>', "printObj": printObj });
            }
    ...
    function PrintFile() {
        ...
        var userSet = $('#<%=hidUserSet.ClientID %>').val();
            ...
            var docVersion = $('#<%=hidDocVersion.ClientID %>').val();
            ...
            var data = [userSet, docID, docVersion, reason, quantity, wdcPrintDate.get_selectedDate()];

            if ($uof.pageMethod.sync("InsertPrintControl", data) == "true") {
                var docMode = $('#<%=hidDocMode.ClientID %>').val();
                    if (docMode == "CONT") {
                        $uof.dialog.open2("~/DMS/DocStore/DocPrintContentByDownLoad.aspx", this, "", 1024, 768, OpenDownloadResult, { "docid": docID, "docversion": docVersion });
                        return false;
                        //top.close();
                    }
                    else {
                        ...
                        $uof.dialog.open2("~/DMS/DocStore/DocDownLoad.aspx", this, "", 700, 550, function (returnValue) {
                            $uof.dialog.close();
                            return true;
                        }, { "docid": '<%= Request["docid"]%>', "status": status, "folderid": '<%= Request["folderid"]%>', "ver": docVersion });
                        return false;
                        //top.close();
                    }
                }
