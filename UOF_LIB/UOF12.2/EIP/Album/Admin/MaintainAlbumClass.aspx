...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    ...
    <script type="text/javascript">
        ...
        function pageLoaded(sender, args) {
            ...
            if ("<%=Request["state"]%>" === "treeSetting") {
                var toolbar = $find("<%=RadToolBar1.ClientID%>");
                var tree = $find("<%= this.albumTree.ClientID %>");
                console.log(tree);
                var treeNode = tree.get_selectedNode();
                var result = $('#<%=lblRadToolBarVisible.ClientID%>').text();

                //判斷是否為管理者
                if (toolbar != null && treeNode != null && result == "True") {

                    //不判斷是否為管理者
                    //if (toolbar != null && treeNode != null ) {
                    {
      ...
      function albumTreeClientNodeClicked(sender, args) {
        ...
        var tree = $find("<%=albumTree.ClientID %>");
    ...
    <asp:UpdatePanel ID="UpdatePanel1" runat="server" RenderMode="Inline" UpdateMode="Conditional">
        <ContentTemplate>
            ...
            <div style="visibility: hidden">
                <asp:Label ID="lblRadToolBarVisible" runat="server" meta:resourceKey="lblRadToolBarVisibleResource1" Text="False"></asp:Label>
            </div>
            </input>
        </ContentTemplate>
    </asp:UpdatePanel>
