...
<asp:Content ID="Content1" ContentPlaceHolderID="ScriptContentPlaceHolder" runat="Server">
    <script type="text/javascript">
        Sys.Application.add_load(function () {
            var is_Editable = "<%=Request["isEditable"]%>";

            //連結會議清單     //其中加入tbx_HiddenField 的目的是用來限定利用傳過來的參數,只可以開窗一次,以避免切換tab時,又造成開窗
            var Meet_Guid = "<%=Request["MeetGuid"]%>";
            if ((Meet_Guid != "") & ($("#<%=tbx_HiddenField.ClientID%>").val() == 'True')) {
                ...
                $uof.dialog.open2("~/EIP/Calendar/CreateMeeting.aspx", "", "", 950, 700,
                    ...
                    { "MeetingGuid": Meet_Guid, "isEditable": is_Editable, "IsMyMeeting": "0" });
            }
            //連結會議清單的被徵詢     //其中加入tbx_HiddenField 的目的是用來限定利用傳過來的參數,只可以開窗一次,以避免切換tab時,又造成開窗
            var Inquiry_Guid = "<%=Request["InquiryGuid"]%>";
            ...
            //連結工作清單
            var work_id = "<%=Request["workid"]%>";
            var Owner_Guid = "<%=Request["OwnerGuid"]%>";
            ...
            //連結交辦清單
            var devolveGuid = "<%=Request["devolveGuid"]%>";
            var Owner_Guid = "<%=Request["OwnerGuid"]%>";
        ...
        <telerik:RadPane ID="RadPane2" runat="server" Height="100%">
            ...
            <asp:UpdatePanel ID="UpdatePanel1" runat="server">
                <ContentTemplate>
                    ...
                    <input id="hideShowEndDate" runat="server" type="hidden" />
                    <input id="hideDisableCreate" runat="server" type="hidden" />
                    <input id="HiddenSelectedTime" runat="server" type="hidden" />
                    <input id="lbOwnerGuid" runat="server" type="hidden" />
                    <input id="hideWhosCalendar" runat="server" type="hidden" />
                    ...
                    <input id="hiddenDecodeXML" runat="server" type="hidden" />
                    <input id="hiddenTabClick" runat="server" type="hidden" />
                    <input id="hiddenTabListClick" runat="server" type="hidden" />
