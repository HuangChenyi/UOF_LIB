...
<script id="CreateBorrowjs" type="text/javascript">
    ...
    function RadToolBar1_CreatBorrow_ButtonClicking(sender, args) {
        ...
        if (Key == "create") {
            ...
            if (borrowValue != '') {
                $uof.dialog.open2("~/EIP/Plant/BorrowEdit.aspx", args.get_item(), "", 1080, 800, openDialogResult, { "borrowPlants": borrowPlantsObj, "borrowGuid": borrowValue, "MeetingStartTime": meetingStartTimeValue, "MeetingEndTime": meetingEndTimeValue, "repeatType": repeatType });
            }
            else {
                $uof.dialog.open2("~/EIP/Plant/BorrowEdit.aspx", args.get_item(), "", 1080, 800, openDialogResult, { "borrowPlants": borrowPlantsObj, "MeetingStartTime": meetingStartTimeValue, "MeetingEndTime": meetingEndTimeValue, "repeatType": repeatType });
            }
        }
        else if (Key == "delete") {
            ...
            args.set_cancel(true);
            var userGuid= $("#<%=hideUserGUID.ClientID%>").val();
            var meetingStartTimeValue = $("#<%=hidMeetingStartTime.ClientID%>").val();
            var meetingEndTimeValue = $("#<%=hidMeetingEndTime.ClientID%>").val();            
            var borrowPlantsObj = "<%=hidBorrowPlants.ClientID%>";     

            $uof.dialog.open2("~/EIP/Plant/ChoiceApprovePlant.aspx", args.get_item(), '<%=lblTitle.Text%>', 800, 600, openDialogResult, { "borrowPlants": borrowPlantsObj, "UserGuid": userGuid, "MeetingStartTime": meetingStartTimeValue, "MeetingEndTime": meetingEndTimeValue });
        }
    }
                ...
                <ContentTemplate>
                    <Fast:Grid ID="Grid1" runat="server" AutoGenerateCheckBoxColumn="True" AutoGenerateColumns="False"
                        ...
                        <Columns>
                            ...
                            <asp:BoundField DataField="BORROW_STATE" HeaderText="狀態" meta:resourcekey="BoundFieldResource4" />
...
<input id="hideBorrowGUID" type="hidden" runat="server" />
<input id="hidBorrowPlants" type="hidden" runat="server" />
<input id="hidMeetingStartTime" type="hidden" runat="server" />
<input id="hidMeetingEndTime" type="hidden" runat="server" />
