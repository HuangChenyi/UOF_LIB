            ...
            <ItemTemplate>
                <div class="uploaderv3item" style="max-width: 99%;" id='<%#Eval("Id") %>'>
                    ...
                    <div style="overflow:hidden; text-overflow:ellipsis; float: left; margin-bottom: 3px;">
                        <a href='<%#Eval("DownloadUrl") %>' onclick='<%#Eval("TagA_OnClientClick") %>'  target='_blank'><%#Eval("Name")%></a>
            ...
            <ClientSettings>
                <DataBinding>
                    ...
                    <div class="uploaderv3item" style="max-width: 99%;" id="#= Id #">
                        ...
                        <div style="overflow:hidden; text-overflow:ellipsis; float: left;">
    ...
    <script type="text/javascript">
        ...
        Ede.Uof.Controls.FileCenter_<%=ClientID%>.prototype = {
            ...
            add_uploaded: function (id, name, path, length, type, url, proxy, ext, iw, ih, groupUofId) {
                ...
                name = name.replace(/\'/g, '');
                var downloadUrl = String.format('<%=FileCenterSetting.DefaultDownloadString%>', url, id, encodeURIComponent(path.replace(/\!/g, '\\')), encodeURIComponent(type), encodeURIComponent(name)); //escape(name)               
            ...
            get_isValidLength: function(length) {
                var limit = $("#<%=hdnTotalSizeKb.ClientID%>").val();
                if (limit === '' || limit === 0) {
                    return true;
                }
        ...
        function popupLink(id) {           
            ...
            radnoti.set_text(String.format("<%=FileCenterSetting.DefaultDownloadPageString%>", id));
            radnoti.show();
        }
...
<telerik:RadScriptBlock ID="RadScriptBlock2" runat="server">
    <script type="text/javascript">
        ...
        function uploadv3_ClientFileUploading_<%=ClientID %>(sender, args) {                    
            ...
            // set request params
            args.set_queryStringParams({ "fgid": v3Obj.fgid, "module": encodeURIComponent(v3Obj.module), "sub": encodeURIComponent(v3Obj.sub), "gid": v3Obj.gid });
        }
