<%@ Control Language="C#" AutoEventWireup="true" Inherits="Common_DCS_UC_XpsViewer" Codebehind="UC_XpsViewer.ascx.cs" %>
...
<script type="text/javascript">
    ...
    $(function() {
        ...
        if (width > 0) {
            var tmpwidth = $(window).width() * 0.7;
            if (wp === "true") {
                if (width < 100) {
                    $xpsvw.width(width + percent);
                    $uimgvw.width(width + percent);
                    $divxps.width(width + percent);
                    $divug.width(width + percent);                   
                }
                ...
                $xpsvw.width(width > 610 ? width : "610"); //min:610    
                $uimgvw.width(width > 610 ? width : "610"); //min:610    
                $divxps.width(width > 610 ? width : "610"); //min:610   
                $divug.width(width > 610 ? width : "610"); //min:610  
                $("#<%=hdnUgWidth.ClientID %>").val(width);
                
            }
            if (Number(width) === 1160 || Number(width) === 100) {
                $select.val('Fit');
        ...
        if (height > 0) {
            if (hp === "true") {
                ...
                $("#<%=hdnUgHeight.ClientID %>").val($(window).height()-70);
            } else {
                $xpsvw.height(height);
...
<asp:UpdatePanel ID="UpdatePanel1" runat="server" ChildrenAsTriggers="True" UpdateMode="Conditional">
    <ContentTemplate>
        <asp:Label ID="lblError" runat="server" Visible="False" ForeColor="White" BackColor="Red" meta:resourcekey="lblErrorResource1"></asp:Label>        
        <asp:Button ID="btnReconvert" runat="server" Text="重新轉檔" OnClick="btnReconvert_Click" Visible="False" meta:resourcekey="btnReconvertResource1"/>   
...
<div class="xps-image-container" style="display: none">
    ...
    <div style="text-align: center; margin-top: 5px;">
        <telerik:RadToolBar ID="RadToolBar1" runat="server"  Visible="False" >
            <Items>
                <telerik:RadToolBarButton runat="server" Text="" ToolTip="Rotate" Value="Rotate" 
                    ...
                    CheckedImageUrl="~/dms/images/docviewer/rotate.png" >
...
<div class="ug-image-container" style="display: none">
    ...
    <div style="text-align: center; margin-top: 5px;">
        <telerik:RadToolBar ID="RadToolBar2" runat="server"  Visible="False">
            <Items>
                <telerik:RadToolBarButton runat="server" Text="" ToolTip="Rotate" Value="Rotate" 
                    DisabledImageUrl="~/dms/images/docviewer/rotate.png"
                    HoveredImageUrl="~/dms/images/docviewer/rotate.png" 
                ...
                <telerik:RadToolBarButton Value="ratioselect">
                        <ItemTemplate>
...
<asp:HiddenField ID="hdnWkfJson" runat="server" Value="[]" />
...
<asp:Label ID="lblSiverLightError" runat="server" Visible="False" Text="Silverlight 應用程式中有未處理的錯誤。 "  meta:resourcekey="lblSiverLightErrorResource1"></asp:Label>
<asp:Label ID="lblErrorCode" runat="server" Visible="False" Text="代碼: "  meta:resourcekey="lblErrorCodeResource1"></asp:Label>
<asp:Label ID="lblErrorType" runat="server" Visible="False" Text="分類: "  meta:resourcekey="lblErrorTypeResource1"></asp:Label>
<asp:Label ID="lblErrorMessage" runat="server" Visible="False" Text="訊息: "  meta:resourcekey="lblErrorMessageResource1"></asp:Label>
<asp:Label ID="lblXamlFile" runat="server" Visible="False" Text="檔案: "  meta:resourcekey="lblXamlFileResource1"></asp:Label>
<asp:Label ID="lblLineNumber" runat="server" Visible="False" Text="行: "  meta:resourcekey="lblLineNumberResource1"></asp:Label>
<asp:Label ID="lblCharPosition" runat="server" Visible="False" Text="位置: "  meta:resourcekey="lblCharPositionResource1"></asp:Label>
<asp:Label ID="lblMethodName" runat="server" Visible="False" Text="方法名稱: "  meta:resourcekey="lblMethodNameResource1"></asp:Label>
</div>
    ...
    <telerik:RadScriptBlock runat="server">
        <script type="text/javascript">
            ...
            function body_ready() {
                ...
                if ($("#<%=hdnUgWidth.ClientID %>").val() !== "") {
                    if ($("#<%=hdnWidthPercentage.ClientID %>").val() === "true")
                        <%=ClientID %>m_UgWidth = ($(window).width() * 0.7) - 10;
                    else
                        <%=ClientID %>m_UgWidth = $("#<%=hdnUgWidth.ClientID %>").val();
                }
                else
                    <%=ClientID %>m_UgWidth = '<%=UgWidth%>';
                
                console.log('width=' +<%=ClientID %>m_UgWidth);
               <%-- if (Number(<%=ClientID %>m_UgWidth) === 1160)
                    <%=ClientID %>m_UgWidth -= 50;--%>
                
                 <%=ClientID %>m_UgHeight = 0;
                console.log('PageRatio=' + '<%=PageRatio%>');
                if ($("#<%=hdnUgHeight.ClientID %>").val() !== "") {
                    <%=ClientID %>m_UgHeight = '<%=PageRatio%>' * <%=ClientID %>m_UgWidth;
                } else {
                    <%=ClientID %>m_UgHeight = '<%=UgHeight%>';
                }

                console.log('m_UgHeight=' +<%=ClientID %>m_UgHeight);
                ...
                setTimeout(function () {
                    var containwidth = $('#<%=ClientID %> .ug-image-container').width();
                <%=ClientID %>m_UgWidth = containwidth;
                    $("#<%=hdnUgWidth.ClientID %>").val(containwidth);
                <%=ClientID %>m_UgHeight = '<%=PageRatio%>' * <%=ClientID %>m_UgWidth;
                ...
                console.log('book length=' + $("#<%=ClientID %> .book").length);
                    $("#<%=ClientID %> .book").each(function (index) {
                    $(this).height(<%=ClientID %>m_UgHeight);
                     $(this).width(<%=ClientID %>m_UgWidth-60 );
            ...
            function initial_page_<%=ClientID %>() {
                ...
                var all_item = [];
                for (var i = 0 ; i < <%=ClientID %>m_NumberOfDocPages ; i++) {
                    var url = get_img_url_<%=ClientID %>(i);
            ...
            function ratiochange_<%=ClientID %>(selectitemid) {
                ...
                $('#<%=ClientID %> .ug-image-container').width($("#<%=hdnUgWidth.ClientID %>").val());
                if ($("#<%=hdnUgWidth.ClientID %>").val() !== "") {
                    UgWidth =$("#<%=hdnUgWidth.ClientID %>").val()-40;
                }
                else
                    UgWidth = '<%=UgWidth%>';
                ...
                var selected_value = $('#' + selectitemid).val();
            ...
            function li_onclickZoom_<%=ClientID %>(trtagId) {
             ...
             //   var trtagOffsetLeft = $('#<%=ClientID %> #' + trtagId).offset().left;
            ...
            function reinitial_page_<%=ClientID %>() {
                ...
                $("#<%=ClientID %> .book").each(function (index) {
                    $(this).height(<%=ClientID %>m_UgHeight);
                     $(this).width(<%=ClientID %>m_UgWidth-20 );
            ...
            function ugitem_load_callback_<%=ClientID %>(item, response, statusText, xhr) {
                ...
                var index = item.index;
                var tdid = item.tdid;
                //var td = $('#<%=ug_content.ClientID %>').find('tbody').children().eq(index).children().eq(0);
                var td = $('#' + tdid).find('tbody').children().eq(index).children().eq(0);
                // 加上已經載入 Cache 區的 IMG
            ...
            function img_onload(img,tdheight,tdwidth) {
                ...
                var w = visualImgObj.width();
                var h = visualImgObj.height();

                if (w > tdwidth) {
                    h = h * tdwidth / w;
                    visualImgObj.width(tdwidth);
                    visualImgObj.height(h);

                    var offset_y = parseInt((tdheight - h) / 2, 10);
                    visualImgObj.position({ top: offset_y, left: visualImgObj.position().left });   // vertical middle
                }
            ...
            function ugimg_onload(img,tdheight,tdwidth) {
                ...
                visualImgObj.css("height", tdheight);
                visualImgObj.css("width", (Number(tdwidth) - 40))
                // 如果 Page 長寬不一致, 則調整長寬比
                var w = visualImgObj.width();
                var h = visualImgObj.height();
            ...
            function ugget_img_url_<%=ClientID %>(page_index) {
                var angle = $('#<%=hdnAngle.ClientID%>').val();
                //return '<%=ResolveUrl("~") %>DMS/Service/GetDocToImageUGHandler.ashx?Index=' + page_index + '&Angle=' + angle + '&xid=' + <%=ClientID %>m_UgId + '&uid=<%=WatermarkUniqueId%>';
            ...
            function show_current_image_<%=ClientID %>() {
                ...
                while (idx < index_list.length) {
                    if (index_list[idx] < 0 || index_list[idx] >= <%=ClientID %>m_NumberOfDocPages)
                ...
                for (var i = 0 ; i < index_list.length; i++) {
                    ...
                    if (need_load === true) {
                        ...
                        // 加入載入清單
                        var url = get_img_url_<%=ClientID %>(index);
                         url += ';' + '<%=xps_content.ClientID %>';
                ...
                if (topIndex < 0)
                    topIndex = 0;
                if (bottomIndex > <%=ClientID %>m_NumberOfDocPages - 1)
                    bottomIndex = <%=ClientID %>m_NumberOfDocPages - 1;
