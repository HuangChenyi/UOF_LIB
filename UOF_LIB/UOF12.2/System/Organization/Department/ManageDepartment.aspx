<%@ Page Language="C#" MasterPageFile="~/Master/DialogMasterPage.master" AutoEventWireup="true" Inherits="System_ManageDepartment" Title="部門管理" Culture="auto" meta:resourcekey="PageResource1" UICulture="auto" Codebehind="ManageDepartment.aspx.cs" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <script type="text/javascript">
        ...
        function ClientNodeDropping(sender, eventArgs) {
            ...
            if (parentNode == null) {
                ...
                var data = [sourceNode.get_value(), sourceNode.get_text(), targetNode.get_value(), sourceNode.get_value(), targetNode.get_value()];
                var result = $uof.pageMethod.sync("ChangeDepartmentParent", data);
        ...
        function ClientNodeEditing(sender, eventArgs) {
            ...
            if (escape(text) == '%A0' || escape(text) == "") {
                ...
                if (node.get_value() == null || node.get_value() == "") {
                    ...
                    var data = [guid, text, parentGuid, guid, oldtext];
                    var result = $uof.pageMethod.sync("CreateDepartment", data);
                    ...
                    var data = [node.get_value(), text, node.get_value(), oldtext];
                    var result = $uof.pageMethod.sync("ModifyDepartment", data);
            ...
            function treeReLoad(result) {
                ...
                if (result.value == null) {
                    ...
                    if (result.value.indexOf('Error') > -1) {
                        ...
                        window.location = "<%=Request.Path %>?focusNode=" + result.value;
                    top.returnValue = "";
                }
            }
        }
        ...
        function RadToolBar1_ManageDep_ButtonClicking(sender, args) {
            ...
            if (Key == "createDept") {
                ...
                var data = [node.get_value(), parentNode.get_value(), index + 1 - 1];
    ...
    <telerik:RadSplitter ID="MainRadSplitter" runat="server" Width="100%" LiveResize="true" ResizeWithBrowserWindow="true" ResizeWithParentPane="true" Orientation="Horizontal" BorderSize="0">
        <telerik:RadPane ID="TopRadPane" runat="server" Height="35px" Scrolling="None">
            <telerik:RadToolBar ID="RadToolBar1" runat="server" Width="100%"
                OnClientButtonClicking="RadToolBar1_ManageDep_ButtonClicking">
                <Items>
                    <telerik:RadToolBarButton runat="server" HoveredImageUrl="~/System/icon_images/icon_m01.gif"
                        ImageUrl="~/System/icon_images/icon_m01.gif" Value="createDept" CheckedImageUrl="~/System/icon_images/icon_m01.gif"
                        Text="新增部門" meta:resourcekey="TBarButtonResource1">
        ...
        <telerik:RadPane ID="TreeRadPane" runat="server" Scrolling="None">
            <telerik:RadTreeView ID="RadTreeView1" runat="server" AllowNodeEditing="true" EnableDragAndDrop="true" OnClientNodeClicking="ClientNodeClicking"
                OnClientNodeClicked="ClientNodeClicked"   Width="494px" Height="459px"
    ...
    <asp:Label ID="lbNeedItemSelected" runat="server" Text="請先選擇節點!" Visible="False"
        meta:resourcekey="lbNeedItemSelectedResource1"></asp:Label>
    <asp:Label ID="lbNotAllowMove" runat="server" Text="無法搬移 ! 不可移動至子節點下" Visible="False"
        meta:resourcekey="lbNotAllowMoveResource1"></asp:Label>
    <asp:Label ID="lbNotAllowDelete" runat="server" Text="不可刪除!" Visible="False" meta:resourcekey="lbNotAllowDeleteResource1"></asp:Label>
    <asp:Label ID="lbNotAllowDelete2" runat="server" Text="節點內仍有項目存在, 不可刪除!" Visible="False"
        meta:resourcekey="lbNotAllowDelete2Resource1"></asp:Label>
