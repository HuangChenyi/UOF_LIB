    ...
    <script type="text/javascript">
        ...
        function GoToNextSite(s) {
            try {
                ...
                if (NextInfo[3] == '') {
                    //location.href = "../FreeTask/SignNodeForm.aspx?TASK_ID=" + NextInfo[0] + "&SITE_ID=" + NextInfo[1] + "&NODE_SEQ=" + NextInfo[2] + "&QUERY_TYPE=" + "<%=Request["QUERY_TYPE"]%>" + "&KEYWORD=" + "<%=Request["KEYWORD"]%>" + "&SORT_STRING=" + "<%=Request["SORT_STRING"]%>" + "&INDEX=" + "<%=Request["INDEX"]%>" + "&CATEGORY_ID=" + "<%=Request["CATEGORY_ID"]%>" + "&AGENT=" + NextInfo[4] + "&GotoNextSite=Y";
                }
    ...
    function signDialogResult(returnValue) {
        ...
        if (typeof (returnValue) == 'undefined') {
            ...
            if (returnValue == "NextSite") {
                ...
                var data = ["<%=Request["TASK_ID"]%>", "<%=Request["SITE_ID"]%>", "<%=Request["NODE_SEQ"]%>", "<%=Request["QUERY_TYPE"]%>", "<%=Request["KEYWORD"]%>", "<%=Request["SORT_STRING"]%>", "<%=Request["INDEX"]%>", "<%=Request["CATEGORY_ID"]%>", "<%=Request["AGENT"]%>"];
                    ...
                    location.href = "../ViewFormNext.aspx?TASK_ID=" + "<%=Request["TASK_ID"]%>" + "&SignedStatus=IsSigned&isMNext=true";
                }
                else {
    ...
    $('#<%=this.HiddenField1.ClientID%>').val($('#<%=this.Panel1.ClientID%>').scrollTop());
        ...
        function OnBarMainClick(sender, args) {
            ...
            if (args.get_item().get_value() == "SaveFormDraft") {
                ...
                args.set_cancel(true);
                $uof.dialog.open2("~/WKF/Browse/ViewMasterFlow.aspx?formVersionId=<%=formVersionId%>&flowId=<%=flowId%>", senderButtton, '<%=dialogTitleName%>', 1280, 1080, openDialogResult);
            }
            else if (args.get_item().get_value() == "ForOpinion") {
                var IS_SCRIPT_IQY = 0;
                var TASK_ID = "<%=hiddenTaskId.Value%>";
                var USER_GUID = "<%=hiddenUserGUID.Value%>";
                var USER_DEPT = "<%=hiddenUserDEPT.Value%>";
                var SITE_ID = "<%=hiddenSiteid.Value%>";
                var NODE_SEQ = "<%=hiddenNodeSEQ.Value%>";
                ...
                args.set_cancel(true);
                $uof.dialog.open2("~/WKF/Browse/ViewSignFlow.aspx?TASK_ID=<%=Request["TASK_ID"]%>&SITE_ID=<%=Request["SITE_ID"]%>&NODE_SEQ=<%=Request["NODE_SEQ"]%>", senderButtton, '<%=dialogTitleName%>', 800, 650, function () { return false; });
            }
                ...
                args.set_cancel(true);
                $uof.dialog.open2("~/WKF/FormUse/SiteSignInfo.aspx?TASK_ID=<%=Request["TASK_ID"]%>&SITE_ID=<%=Request["SITE_ID"]%>", senderButtton, '<%=dialogTitleName%>', 800, 650, function () { return false; });
            }
            else if (args.get_item().get_value() == "SignComment") {

                var ScriptID = "<%=Request["TASK_ID"]%>";
                var SiteID = "<%=Request["SITE_ID"]%>";
                var NodeSEQ = "<%=Request["NODE_SEQ"]%>";
                var IsScript = "N";
                args.set_cancel(true);
                ...
                args.set_cancel(true);
                $uof.dialog.open2("~/WKF/FormManagement/EditFormDescription.aspx", senderButtton, "", 1024, 900, function () { return false; }, { "formVersionId": "<%=formVersionId%>", "type": "View", "culture": "<%=Ede.Uof.EIP.SystemInfo.Current.Culture%>" });
            }
}
