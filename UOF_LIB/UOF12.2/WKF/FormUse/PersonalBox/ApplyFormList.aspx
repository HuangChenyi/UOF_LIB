...
<asp:Content ID="Content1" ContentPlaceHolderID="ScriptContentPlaceHolder" runat="Server">
    <script id="ApplyFormListjs" type="text/javascript">
        var formTreeClientId = "<%=formTreeClientId %>";
        var initialLoad = true;
        Sys.Application.add_load(function () {
            if (initialLoad && "<%=Request["fillFormDirectly"]%>" === "true" && "<%=check%>" === "true") {

                var signWindowWidth = "<%=signWindowWidth%>";
                var signWindowHeight = "<%=signWindowHeight%>";
                var AddFormScriptParams = { "formVersionId": "<%=formVersionId%>", "formId": "<%=Request["formId"]%>" };
                ...
                var node = tree.findNodeByValue("<%=Request["formId"]%>" + "@" + "<%=formVersionId%>");
                initialLoad = false;
                node.select();
            }
        });
        ...
        {
            var AddFormScriptParams={"formVersionId":GetFormVersionId(),"formId":GetFormId()};
