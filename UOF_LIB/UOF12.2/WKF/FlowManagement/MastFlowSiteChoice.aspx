    ...
    function rbClick(i)
    {
        ...
        function ChoicePopDialog() {
            ...
            if ($("#ctl00_ContentPlaceHolder1_rbAssignableSite").is(":checked")) {
                $uof.dialog.open2("<%=signableSiteUrl%>", this, "<%=lblAddMsg_Sign.Text%>", 700, 550, openDialogResult);
                return false;
            }

            //流程式            
            if ($("#ctl00_ContentPlaceHolder1_rbFlowSite").is(":checked")) {
                $uof.dialog.open2("<%=flowSiteUrl%>", this, "<%=lblAddMsg_Flow.Text%>", 480, 600, openDialogResult);
                return false;
            }

            //條件式站點
            if ($("#ctl00_ContentPlaceHolder1_rbConditionBranchSite").is(":checked")) {
                $uof.dialog.open2("<%=conditionBranchSiteUrl%>", this, "<%=lblAddMsg_Cond.Text%>", 940, 600, openDialogResult);
                return false;
            }

            //自選流程站點
            if ($("#ctl00_ContentPlaceHolder1_rbGroupSite").is(":checked")) {
                $uof.dialog.open2("<%=groupFlowSiteUrl%>", this, "<%=lblAddMsg_CustGroup.Text%>", 760, 580, openDialogResult);
                return false;
            }

            //表單欄位站點
            if ($("#ctl00_ContentPlaceHolder1_rbFieldSite").is(":checked")) {
                $uof.dialog.open2("<%=fieldFlowSiteUrl%>", this, "<%=lblAddMsg_Field.Text%>", 760, 580, openDialogResult);
                return false;
            }

            //應用程式站點
            if ($("#ctl00_ContentPlaceHolder1_rbExternalSite").is(":checked")) {
                $uof.dialog.open2("<%=externalFlowSiteUrl%>", this, "<%=lblAddMsg_External.Text%>", 600, 150, openDialogResult);
                return false;
            }

            //知會站點
            if ($("#ctl00_ContentPlaceHolder1_rbAlertSite").is(":checked")) {
                $uof.dialog.open2("<%=alertSiteUrl%>", this, "<%=lblAddMsg_External.Text%>", 760, 580, openDialogResult);
                return false;
            }
        }
