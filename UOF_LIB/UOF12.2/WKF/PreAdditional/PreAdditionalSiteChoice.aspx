...
<script language=javascript>
    ...
    function ChoicePopDialog() {
        ...
        if ($("#ctl00_ContentPlaceHolder1_rbAssignableSite").prop("checked")) {
            $uof.dialog.open2('<%=signableSiteUrl%>', this, '', 700, 550, openDialogResult);
            return false;
        }
        //自訂流程
        if ($("#ctl00_ContentPlaceHolder1_rbCustSite").prop("checked")) {
            $uof.dialog.open2('<%=custSiteUrl%>', this, '', 1280, 1080, openDialogResult);
            return false;
        }

        //知會
        if ($("#ctl00_ContentPlaceHolder1_rbAlertSite").prop("checked")) {
            $uof.dialog.open2('<%=alertSiteUrl%>', this, '', 760, 580, openDialogResult);
            return false;
        }
    }
